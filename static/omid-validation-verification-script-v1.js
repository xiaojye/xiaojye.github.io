...
// Send OMID supported event after a delay because it comes too early and gets ignored.
  if (typeof window === "undefined") {
    console.log("Oops, `window` is not defined cannot send anjam event")
  }else if (window.anjam) {
    setTimeout(function(){
      anjam.DispatchAppEvent("OMIDEvent","OmidSupported[" + a + "]");
    }, 2000);
  }

  a && (this.verificationClient_.registerSessionObserver(function(d) {
    return c.sessionObserverCallback_(d);
  }, b), Object.keys(module$exports$omid$common$constants.AdEventType).filter(function(d) {
    return module$exports$omid$common$constants.AdEventType[d] !== module$exports$omid$common$constants.AdEventType.MEDIA && module$exports$omid$common$constants.AdEventType[d] !== module$exports$omid$common$constants.AdEventType.VIDEO;
  }).forEach(function(d) {
    return c.verificationClient_.addEventListener(module$exports$omid$common$constants.AdEventType[d], function(e) {
      return c.omidEventListenerCallback_(e);
    });
  }));
};
module$exports$omid$validationVerificationScript$ValidationVerificationClient.prototype.logMessage_ = function(a, b) {
  ...
  // Dispatch all events to OMID as they arrive.
  if (typeof window === "undefined") {
    console.log("Oops, `window` is not defined cannot send anjam event")
  }else if (window.anjam) {
      anjam.DispatchAppEvent("OMIDEvent",a);
  }
